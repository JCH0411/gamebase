...
public:
	...
	{
		string gName;
		vector <gameClass>* ptr = new vector<gameClass>;
		ptr = driverObj.getGameVector();
		...
		/*vector <gameClass>tempVector = driverObj.getGameVector();                                  // commenetd to add a ponter
		int x = 0;
		int y;*/
		...
		do
		{
			...
			case 1:
				...
				cin >> tempString;
				tempVector[x].getgenre = tempString;
				break;
			case 2:
	...
	void dispADAdd()
	{
		...
		driverObj.createNewGame(name, hardware, year, price, genre, rating, attribute);
	}
};
#endif
